...
**Known Gap — Allocation Validation:** Exact Amount and Percentage modes do not currently validate that entries add up to the expense total before saving. There's no running total/remainder display and no submit-lock in these modes today — an under- or over-allocated split (e.g., percentages summing to 90%) is currently silently saved as entered (with Percentage mode's last-entry logic potentially producing a negative amount if the earlier entries overshoot 100%). This is a known, not-yet-decided-on gap — see prioritization discussion with the team before treating it as a spec.
